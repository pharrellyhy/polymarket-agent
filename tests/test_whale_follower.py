...
def _make_trader(name: str = "whale1", rank: int = 1) -> Trader:
    return Trader(rank=rank, name=name, volume=100000.0, pnl=5000.0, markets_traded=50)
...
def test_whale_follower_generates_signal() -> None:
    ...
    # Mock gamma client to return a large trade
    strategy._gamma.get_trader_activity = MagicMock(
        return_value=[
            {"market": "100", "asset": "0xtok_100_yes", "side": "buy", "size": 500, "price": 0.5, "timestamp": "now"}
        ]
    )

    markets = [_make_market("100")]
    signals = strategy.analyze(markets, data)
    assert len(signals) == 1
    assert signals[0].side == "buy"
    assert signals[0].market_id == "100"
...
def test_whale_follower_no_signal_small_trade() -> None:
    ...
    strategy._gamma.get_trader_activity = MagicMock(
        return_value=[{"market": "100", "asset": "0xtok_100_yes", "side": "buy", "size": 50, "price": 0.5}]
    )
...
def test_whale_follower_deduplicates() -> None:
    ...
    trades = [
        {"market": "100", "asset": "0xtok_100_yes", "side": "buy", "size": 500, "price": 0.5},
        {"market": "100", "asset": "0xtok_100_yes", "side": "buy", "size": 600, "price": 0.5},
    ]
    strategy._gamma.get_trader_activity = MagicMock(return_value=trades)
...
def test_whale_follower_confidence_inversely_proportional_to_rank() -> None:
    ...
    strategy._gamma.get_trader_activity = MagicMock(
        side_effect=[
            [{"market": "100", "asset": "tok1", "side": "buy", "size": 500, "price": 0.5}],
            [{"market": "200", "asset": "tok2", "side": "buy", "size": 500, "price": 0.5}],
        ]
    )

    markets = [_make_market("100"), _make_market("200")]
    signals = strategy.analyze(markets, data)

    if len(signals) == 2:
        rank1_signal = next(s for s in signals if s.market_id == "100")
        rank10_signal = next(s for s in signals if s.market_id == "200")
...
def test_whale_follower_skips_unknown_market() -> None:
    ...
    strategy._gamma.get_trader_activity = MagicMock(
        return_value=[{"market": "999", "asset": "tok_unknown", "side": "buy", "size": 500, "price": 0.5}]
    )

    markets = [_make_market("100")]  # Market 999 not in list
    signals = strategy.analyze(markets, data)
    assert len(signals) == 0
...
def test_whale_follower_does_not_dedup_before_market_match() -> None:
    ...
    strategy._gamma.get_trader_activity = MagicMock(
        return_value=[{"market": "999", "asset": "0xtok_999_yes", "side": "buy", "size": 500, "price": 0.5}]
    )

    # First run: market 999 is not active, so no signal should be emitted.
    assert strategy.analyze([_make_market("100")], data) == []

    # Second run: once market 999 is active, the same trade should still produce a signal.
    signals = strategy.analyze([_make_market("999")], data)
    assert len(signals) == 1
    assert signals[0].market_id == "999"
